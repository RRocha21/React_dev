...
async function createAndroidPackagePaths(filePath, packageName) {
  const {
    ...
    segments,
    packageNameArray
  } = getPackageNameDetails(packageName);
  ...
  if (pathFolders[0] === 'java' && pathFolders[1] === 'com') {
    ...
    if (segmentsList.length > 1) {
      ...
      try {
        await _fsExtra().default.rename(`${filePath}/${segmentsList.join('.')}`, `${filePath}/${segmentsList[segmentsList.length - 1]}`);
        for (const segment of segmentsList) {
          _fsExtra().default.mkdirSync(segment);
          process.chdir(segment);
        }
        await _fsExtra().default.rename(`${filePath}/${segmentsList[segmentsList.length - 1]}`, process.cwd());
      } catch {
...
async function replacePlaceholderWithPackageName({
  ...
  for (const filePath of (0, _walk.default)(process.cwd()).reverse()) {
    ...
    if (!(await _fsExtra().default.stat(filePath)).isDirectory()) {
      ...
      await replaceNameInUTF8File(filePath, `PRODUCT_BUNDLE_IDENTIFIER = "${startsWithCom ? cleanPackageName : `com.${cleanPackageName}`}"`, 'PRODUCT_BUNDLE_IDENTIFIER = "(.*)"');
